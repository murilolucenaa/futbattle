...
lib/data/stadiums.json  # 227 estádios reais de todas as Copas (formato, cores da arquibancada, fosso/pista, detalhe)
lib/data/stadiums.ts    # Perfis tipados: StandShape (oval/circular/rect/rectNfl/horseshoe/dome), roof, track, cores hex; stadiumProfile(name, year?)
...
- Campo renderizado em **PixiJS** (`MatchStage.tsx`): 22 agentes com steering (âncora tática + perseguição da bola pelos 2 mais próximos + separação), ~1400 pontos de torcida nas cores dos kits (atrás de cada gol só a torcida do dono; mudam de lado no espelhamento do 2º tempo), goleiros com cor automática de alto contraste (`pickGkColors`). **Arquibancada por estádio** (`drawStands`): recebe `stadium: StadiumProfile` (de `stadiums.ts`) e pinta o bowl com as cores reais das cadeiras + 2º anel, **fosso/pista** quando `track` (cor ferrugem p/ pista, cinza p/ fosso), sombra de **cobertura** (dome/tent/partial) e fundo aberto na **ferradura**. A silhueta **morfa de verdade** (`presentation/bowl.ts`, testado): superelipse `|x/rx|^n + |y/ry|^n = 1` com expoente por forma (rect boxy → rectNfl → oval → circular elipse pura), ferradura colapsa o anel no gol adversário (Marathon Gate), dome = oval fechada. Bowl, fosso, cobertura e os ~1400 dots de torcida são todos **anéis radiais** entre o retângulo do gramado e essa silhueta (mesmas `innerRadius`/`outerRadius`/`bandTheta`/`ringPoint`), então sempre concordam nas bordas. O gramado é desenhado **por cima** do bowl (mascara o centro → vira anel); cantos fora da silhueta ficam escuros. QA visual: `scripts/preview-stadium.mts` rasteriza PNGs reusando o módulo. Nome só no jogador com bola + hover. No 2º tempo o render é **espelhado** (`snapshot().mirrored`). Coordenadas internas seguem a engine (x 0→100 = ataque do mandante); espelhar só no draw.
...
- **Mobile/responsivo** (touch-first): `layout.tsx` exporta `viewport` (viewport-fit cover, pinch-zoom mantido). Foundation em globals.css — `touch-action: manipulation` global (sem delay/zoom no duplo-toque), helpers `.safe-t/.safe-b/.safe-x/.safe-y`, `.screen`/`.scroll-x`/`.scroll-y`, sombras de tinta suavizadas <640px, `@media (hover:none)` mata `:hover`. Padrões: telas viram **scroll do body** com `my-auto` pra centralizar sem cortar em telas baixas; modais full-screen usam `overflow-y-auto` + `my-auto` + `.safe-y`; listas com scroll interno ganham `max-h-[Nvh]` no mobile e só `lg:flex-1 lg:min-h-0` no desktop (senão `flex-1 min-h-0` colapsa sem altura fixa de pai). Tabelas densas (grupos) escondem colunas secundárias com `hidden sm:table-cell`. Draft (`GameShell`) empilha em 1 coluna — escalar funciona por **duplo-toque no nome** (não precisa ver o campo). Pré-jogo reordena no mobile (VS/estádio/kit + Apito primeiro, via `order-*`). Partida é **100% vertical** (HUD já empilhado; campo Pixi em 4:3 no mobile, 16:10 no desktop).
